...
/// ・★ nonExplodeOnHitLayers：このレイヤーに触れても爆発しない（Trigger/Collision 共通）
/// </summary>
...
public class BulletHoming : MonoBehaviour
{
	...
	// =======================
	[Header("当たっても爆発しない設定")]
	[SerializeField, Tooltip("このレイヤーに触れても（Trigger/Collision問わず）爆発しない")]
	private LayerMask nonExplodeOnHitLayers;

	[SerializeField, Tooltip("無視対象に触れたとき弾を破壊する（falseなら飛び続ける）")]
	private bool destroyOnIgnoredHit = false;
	...
	{
		if (m_explodedOrDestroyed) return;
		if (!explodeOnCollision) return;

		GameObject otherGo = collision.collider.attachedRigidbody != null
			? collision.collider.attachedRigidbody.gameObject
			: collision.collider.gameObject;

		// ★「当たっても爆発しない」レイヤー：trueなら爆発しない
		if (IsInLayerMask(otherGo.layer, nonExplodeOnHitLayers))
		{
			if (destroyOnIgnoredHit)
			{
				// 無視対象に触れたら破壊（爆発はしない）
				PlayEffectsAndDestroy(); // 演出だけ出すならこれ / Destroy(gameObject) にしてもOK
			}
			// 何もしないでスルー（飛び続ける）
			return;
		}
	...
	private void OnTriggerEnter(Collider other)
	{
		...
		GameObject otherGo = other.attachedRigidbody != null
		...
		if (IsInLayerMask(otherGo.layer, nonExplodeOnHitLayers))
		{
			...
			{
				// 無視対象に触れたら破壊（爆発はしない）
				PlayEffectsAndDestroy(); // または Destroy(gameObject);
			}
			return; // スルー
		}
	...
	/// <summary>Runtime で「爆発しないレイヤー」を設定/上書きしたい時用</summary>
	public void SetNonExplodeOnHitLayers(LayerMask mask)
	{
		nonExplodeOnHitLayers = mask;
	}
	...
	private void UpdateOrientation()
	{
		...
		if (aimSlerpSpeed > 0f)
		{
			float t = 1f - Mathf.Exp(-aimSlerpSpeed * Time.fixedDeltaTime);
